...
function sortBIMObjectByModel(arrayOfBIMObject) {
  let arrayModel = [];
  for (const key in spinal.BimObjectService.mappingNameByModel) {
    if (spinal.BimObjectService.mappingNameByModel.hasOwnProperty(key)) {
      const model = spinal.BimObjectService.mappingNameByModel[key];
      arrayModel.push({
        selection: [],
        model
      });
...
let dataService = {
  ...
  async getFloor() {
    ...
    let context = await graph.SpinalGraphService.getContext(
      geographicConstants.FLOOR_REFERENCE_CONTEXT);
  ...
  getStepProcess(stepNode) {
    return loadModel(stepNode.parents.SpinalSystemServiceTicketHasStep[0].ptr).then((relation) => {
      // return relation.parent
      return relation.parent.load();
    });
  },
  ...
  async getAllData() {
    ...
    console.log("ticketsByProcess", ticketsByProcess);
